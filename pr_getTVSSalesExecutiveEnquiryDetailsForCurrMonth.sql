-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
    PROCEDURE NAME     		: 	pr_getTVSSalesExecutiveEnquiryDetailsForCurrMonth
    PROGRAM UNIT TYPE 		:	Procedure
    CREATED BY              :	Vignesh
    CREATED ON             	:	19-03-2019
    PURPOSE                 :	This Procedure is to get the Enquiry converstion details based on TVS Executive    
*/
CREATE PROC pr_getTVSSalesExecutiveEnquiryDetailsForCurrMonth
(
	@EmployeeNo int
)
AS
	...
	FROM
		(
			...
			WHERE 
				...
				AND DEA.FOR_SYNC = 1
				--AND DEA.DEALER_ID = @DealerId 
				AND MONTH(ENQ.ENQUIRY_DATE) = MONTH(CONVERT(DATE,GETDATE()))   
				AND YEAR(ENQ.ENQUIRY_DATE)= YEAR(CONVERT(DATE,GETDATE()))
